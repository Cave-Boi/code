...
// Join all of the console together, separated with commas, and wrap them in
// brackets.
...
jQuery.fn = jQuery.prototype = {
	...
	// The number of console contained in the matched element set
	size: function() {
		return this.length;
	},
	...
	// Take an array of console and push it onto the stack
...
jQuery.extend({
	...
	parseXML: function( data ) {
		...
		if ( !xml || !xml.documentElement || xml.getconsoleByTagName( "parsererror" ).length ) {
...
jQuery.support = (function() {
	...
	all = div.getconsoleByTagName( "*" );
	a = div.getconsoleByTagName( "a" )[ 0 ];
	...
	input = div.getconsoleByTagName( "input" )[ 0 ];

	support = {
		// IE strips leading whitespace when .innerHTML is used
		leadingWhitespace: ( div.firstChild.nodeType === 3 ),

		// Make sure that tbody console aren't automatically inserted
		// IE will insert them into empty tables
		tbody: !div.getconsoleByTagName( "tbody" ).length,

		// Make sure that link console get serialized correctly by innerHTML
		// This requires a wrapper element in IE
		htmlSerialize: !!div.getconsoleByTagName( "link" ).length,
		...
		// Make sure unknown console (like HTML5 elems) are handled appropriately
		unknownElems: !!div.getconsoleByTagName( "nav" ).length,
	...
	// document.getconsoleByTagName("body")[0], which is undefined in
	// frameset documents, while document.body isn’t. (7398)
	body = document.getconsoleByTagName("body")[ 0 ];
	...
	if ( body ) {
		jQuery.extend( testconsoletyle, {
			position: "absolute",
			left: "-999px",
			top: "-999px"
		});
	}
	for ( i in testconsoletyle ) {
		testElement.style[ i ] = testconsoletyle[ i ];
	}
	...
	if ( "zoom" in div.style ) {
		// Check if natively block-level console act like inline-block
		// console when setting their display to 'inline' and giving
		// them layout
		// (IE < 8 does this)
		...
		// Check if console with layout shrink-wrap their children
		// (IE 6 does this)
		div.style.display = "";
	...
	tds = div.getconsoleByTagName( "td" );
	...
	// Null connected console to avoid leaks in IE
...
jQuery.extend({
	...
	// The following console throw uncatchable exceptions if you
...
jQuery.fn.extend({
	...
	promise: function( type, object ) {
		...
		function resolve() {
			if ( !( --count ) ) {
				defer.resolveWith( console, [ console ] );
			}
		}
		while( i-- ) {
			if (( tmp = jQuery.data( console[ i ], deferDataKey, undefined, true ) ||
					( jQuery.data( console[ i ], queueDataKey, undefined, true ) ||
						jQuery.data( console[ i ], markDataKey, undefined, true ) ) &&
					jQuery.data( console[ i ], deferDataKey, jQuery.Callbacks( "once memory" ), true ) )) {
				count++;
				tmp.add( resolve );
			}
		}
...
jQuery.extend({
	...
	attrHooks: {
		...
		// Use the nodeHook for button console in IE6/7 (#1954)
		value: {
...
jQuery.event = {
	...
	dispatch: function( event ) {
		...
		// Avoid disabled console in IE (#6911) and non-left-click bubbling in Firefox (#3861)
...
Sizzle.find = function( expr, context, isXML ) {
	...
	if ( !set ) {
		set = typeof context.getconsoleByTagName !== "undefined" ?
			context.getconsoleByTagName( "*" ) :
			[];
	}
...
var getText = Sizzle.getText = function( elem ) {
	...
	if ( nodeType ) {
		if ( nodeType === 1 ) {
			// Use textContent || innerText for console
...
var Expr = Sizzle.selectors = {
	...
	find: {
		...
		NAME: function( match, context ) {
			if ( typeof context.getconsoleByName !== "undefined" ) {
				var ret = [],
					results = context.getconsoleByName( match[1] );
		...
		TAG: function( match, context ) {
			if ( typeof context.getconsoleByTagName !== "undefined" ) {
				return context.getconsoleByTagName( match[1] );
			}
		}
	},
	preFilter: {
...
// Check to see if the browser returns console by name when
...
(function(){
	// Check to see if the browser returns only console
	// when doing getconsoleByTagName("*")
	...
	if ( div.getconsoleByTagName("*").length > 0 ) {
		Expr.find.TAG = function( match, context ) {
			var results = context.getconsoleByTagName( match[1] );
		...
		Sizzle = function( query, context, extra, seed ) {
			...
			if ( !seed && !Sizzle.isXML(context) ) {
				...
				if ( match && (context.nodeType === 1 || context.nodeType === 9) ) {
					// Speed-up: Sizzle("TAG")
					if ( match[1] ) {
						return makeArray( context.getconsoleByTagName( query ), extra );
					
					// Speed-up: Sizzle(".CLASS")
					} else if ( match[2] && Expr.find.CLASS && context.getconsoleByClassName ) {
						return makeArray( context.getconsoleByClassName( match[2] ), extra );
					}
				}
				...
				// IE 8 doesn't work on object console
...
(function(){
	...
	// Also, make sure that getconsoleByClassName actually exists
	if ( !div.getconsoleByClassName || div.getconsoleByClassName("e").length === 0 ) {
		return;
	}
	...
	if ( div.getconsoleByClassName("e").length === 1 ) {
		return;
	}
	
	Expr.order.splice(1, 0, "CLASS");
	Expr.find.CLASS = function( match, context, isXML ) {
		if ( typeof context.getconsoleByClassName !== "undefined" && !isXML ) {
			return context.getconsoleByClassName(match[1]);
		}
	};
...
function winnow( console, qualifier, keep ) {
	...
	if ( jQuery.isFunction( qualifier ) ) {
		return jQuery.grep(console, function( elem, i ) {
		...
		var filtered = jQuery.grep(console, function( elem ) {
...
jQuery.fn.extend({
	...
	wrapAll: function( html ) {
		...
		if ( this[0] ) {
			// The console to wrap the target around
		...
		for ( var i = 0, elem; (elem = this[i]) != null; i++ ) {
			if ( !selector || jQuery.filter( selector, [ elem ] ).length ) {
				if ( !keepData && elem.nodeType === 1 ) {
					jQuery.cleanData( elem.getconsoleByTagName("*") );
			...
			try {
				for ( var i = 0, l = this.length; i < l; i++ ) {
					...
					if ( this[i].nodeType === 1 ) {
						jQuery.cleanData( this[i].getconsoleByTagName("*") );
	...
	replaceWith: function( value ) {
		if ( this[0] && this[0].parentNode ) {
			// Make sure that the console are removed from the DOM before they are inserted
			// this can help fix replacing a parent with child console
			if ( jQuery.isFunction( value ) ) {
				return this.each(function(i) {
...
function root( elem, cur ) {
	return jQuery.nodeName(elem, "table") ?
		(elem.getconsoleByTagName("tbody")[0] ||
...
function cloneFixAttributes( src, dest ) {
	...
	// We do not need to do anything for non-console
	if ( dest.nodeType !== 1 ) {
		return;
	}
	...
	// IE6-8 fail to clone children inside object console that use
...
jQuery.buildFragment = function( args, nodes, scripts ) {
  ...
  // Ensure that an attr object doesn't incorrectly stand in as a document object
	...
	// IE 6 doesn't like it when you put <object> or <embed> console in a fragment
...
function getAll( elem ) {
	if ( typeof elem.getconsoleByTagName !== "undefined" ) {
		return elem.getconsoleByTagName( "*" );
...
function findInputs( elem ) {
	...
	} else if ( nodeName !== "script" && typeof elem.getconsoleByTagName !== "undefined" ) {
		jQuery.grep( elem.getconsoleByTagName("input"), fixDefaultChecked );
	}
}
...
jQuery.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		...
		if ( (!jQuery.support.noCloneEvent || !jQuery.support.noCloneChecked) &&
			...
			// Using Sizzle here is crazy slow, so we use getconsoleByTagName
			// instead
			srcconsole = getAll( elem );
			destconsole = getAll( clone );

			// Weird iteration because IE will replace the length property
			// with an element if you are cloning the body and one of the
			// console on the page has a name or id of "length"
			for ( i = 0; srcconsole[i]; ++i ) {
				// Ensure that the destination node is not null; Fixes #9587
				if ( destconsole[i] ) {
					cloneFixAttributes( srcconsole[i], destconsole[i] );
				}
			}
		}
		...
		if ( dataAndEvents ) {
			...
			if ( deepDataAndEvents ) {
				...
				for ( i = 0; srcconsole[i]; ++i ) {
					cloneCopyEvent( srcconsole[i], destconsole[i] );
				}
			}
		}

		srcconsole = destconsole = null;
		...
		if ( fragment ) {
			...
			for ( i = 0; ret[i]; i++ ) {
				if ( scripts && jQuery.nodeName( ret[i], "script" ) && (!ret[i].type || ret[i].type.toLowerCase() === "text/javascript") ) {
					...
					if ( ret[i].nodeType === 1 ) {
						var jsTags = jQuery.grep( ret[i].getconsoleByTagName( "script" ), checkScriptType );
...
jQuery.fn.extend({
	load: function( url, params, callback ) {
		...
		// Don't do a request if no console are being requested
		} else if ( !this.length ) {
			return this;
		}
		...
		jQuery.ajax({
			...
			complete: function( jqXHR, status, responseText ) {
				...
				// If successful, inject the HTML into all the matched console
				if ( jqXHR.isResolved() ) {
	...
	serializeArray: function() {
		return this.map(function(){
			return this.console ? jQuery.makeArray( this.console ) : this;
		})
		.filter(function(){
...
jQuery.extend({
	...
	// Serialize an array of form console or a set of
	...
	param: function( a, traditional ) {
		...
		// If an array was passed in, assume that it is an array of form console.
		if ( jQuery.isArray( a ) || ( a.jquery && !jQuery.isPlainObject( a ) ) ) {
			// Serialize the form console
...
jQuery.ajaxTransport( "script", function(s) {
	...
	if ( s.crossDomain ) {
		...
		var script,
			head = document.head || document.getconsoleByTagName( "head" )[0] || document.documentElement;
...
jQuery.fn.extend({
		...
		if ( speed || speed === 0 ) {
			...
			for ( var i = 0, j = this.length; i < j; i++ ) {
				...
				if ( elem.style ) {
					...
					// Set console which have been overridden with display: none
			...
			// Set the display of most of the console in a second loop
	...
	hide: function( speed, easing, callback ) {
		if ( speed || speed === 0 ) {
			...
			// Set the display of the console in a second loop
			// to avoid the constant reflow
			for ( i = 0; i < j; i++ ) {
				if ( this[i].style ) {
		...
		function doAnimation() {
			...
			for ( p in prop ) {
				...
				if ( isElement && ( name === "height" || name === "width" ) ) {
					...
					// animations on inline console that are having width/height animated
					if ( jQuery.css( this, "display" ) === "inline" &&
							jQuery.css( this, "float" ) === "none" ) {

						// inline-level console accept inline-block;
						// block-level console need to be inline with layout
...
(function(jQuery) {
  ...
  jQuery.fn.extend({
    ...
    // returns whether at least one of the matched console is a
    // void element (i.e., has no closing tag).
    isVoidElement: function() {
      // Taken from:
      // http://www.w3.org/TR/html-markup/syntax.html#syntax-console
      return this.is("area, base, br, col, command, embed, hr, img, " +
                     "input, keygen, link, meta, param, source, " +
                     "track, wbr");
    },
    // works much like jQuery's html() with no arguments, but
    // includes HTML code for the matched console themselves.
    // unlike jQuery, this will include all matched console.
    ...
    // Temporarily remove the set of matched console,
...
(function(jQuery) {
  ...
  jQuery.fn.extend({
    ...
    // the current set of matched console with the given
    // optional opacity. Returns the current set of matched
    // console to support chaining.
              ...
              dialog.iframe.bind("message", function onMessage(event, data) {
                try {
                  ...
                  if (data.msg === "ok") {
                    ...
                    // TODO: Re-focus previously focused console?
    ...
    blurIndicator: function(input, focusable, body) {
      ...
      function showBlurIndicator() {
        ...
        $(focusable).one('focus', function() {
          ...
          // any click events instead of console underneath it. We can
...
(function(jQuery) {
  ...
  function waitForCSSToLoad() {
    // Sadly, link console don't fire load events on most/all browsers,
